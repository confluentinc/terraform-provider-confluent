output "resource-ids" {
  value = <<-EOT
  Source Kafka Cluster's Environment ID:   ${confluent_kafka_cluster.source-cluster.environment.0.id}
  Source Kafka Cluster ID: ${confluent_kafka_cluster.source-cluster.id}
  Source topic name: ${confluent_kafka_topic.orders.topic_name}

  Destination Kafka Cluster's Environment ID:   ${confluent_kafka_cluster.destination-cluster.environment.0.id}
  Destination Kafka Cluster ID: ${confluent_kafka_cluster.destination-cluster.id}
  ...
  * Many of the mirror topic’s configurations are copied and synced from the source topic. A full list is at the end of this page.
  ...
  In order to use the Confluent CLI v2 to produce to source topic '${confluent_kafka_topic.orders.topic_name}' and consume from mirror topic '${confluent_kafka_topic.orders.topic_name}' using Kafka API Keys
  ...
  # 2. Produce key-value records to source topic '${confluent_kafka_topic.orders.topic_name}' by using ${confluent_service_account.app-manager-source-cluster.display_name}'s Kafka API Key
  $ confluent kafka topic produce ${confluent_kafka_topic.orders.topic_name} --environment ${confluent_kafka_cluster.source-cluster.environment.0.id} --cluster ${confluent_kafka_cluster.source-cluster.id} --api-key "${confluent_api_key.app-manager-source-cluster-api-key.id}" --api-secret "${confluent_api_key.app-manager-source-cluster-api-key.secret}"
  ...
  # 3. Consume records from a mirror topic '${confluent_kafka_topic.orders.topic_name}' (created by and owned by a cluster link '${confluent_cluster_link.destination-outbound.link_name}') by using ${confluent_service_account.app-manager-destination-cluster.display_name}'s Kafka API Key
  $ confluent kafka topic consume ${confluent_kafka_topic.orders.topic_name} --from-beginning --environment ${confluent_kafka_cluster.destination-cluster.environment.0.id} --cluster ${confluent_kafka_cluster.destination-cluster.id} --api-key "${confluent_api_key.app-manager-destination-cluster-api-key.id}" --api-secret "${confluent_api_key.app-manager-destination-cluster-api-key.secret}"
