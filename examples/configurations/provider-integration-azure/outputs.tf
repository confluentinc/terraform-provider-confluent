...
output "provider_integration_status" {
  ...
  value       = confluent_provider_integration_setup.azure.status
}
...
output "azure_admin_consent_url" {
  ...
  value       = "https://login.microsoftonline.com/${var.azure_tenant_id}/adminconsent?client_id=${confluent_provider_integration_authorization.azure.azure[0].confluent_multi_tenant_app_id}"
}