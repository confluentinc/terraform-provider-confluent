...
resource "confluent_role_binding" "statements-runner-assigner" {
...
resource "confluent_role_binding" "app-manager" {
...
resource "confluent_flink_compute_pool" "main" {
  ...
  depends_on = [
    confluent_role_binding.statements-runner-assigner,
    confluent_api_key.app-manager-flink-api-key,
    confluent_role_binding.app-manager,
  ]
}
...
resource "confluent_flink_statement" "insert-into-table" {
  ...
  statement  = "INSERT INTO random_int_table VALUES (CURRENT_TIMESTAMP, RAND_INTEGER(100)), (CURRENT_TIMESTAMP, RAND_INTEGER(1000));"
  properties = {
    "sql.current-catalog"  = data.confluent_environment.staging.display_name
    "sql.current-database" = confluent_kafka_cluster.standard.display_name
  }
...
resource "confluent_flink_statement" "select-from-table" {
  ...
  statement  = "SELECT * FROM random_int_table;"
  properties = {
    "sql.current-catalog"  = data.confluent_environment.staging.display_name
    "sql.current-database" = confluent_kafka_cluster.standard.display_name
  }
