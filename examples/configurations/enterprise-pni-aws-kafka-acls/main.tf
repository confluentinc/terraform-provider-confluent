...
# Output EC2 public IP and connection commands
output "ssh_setup_instructions" {
  value       = <<-EOT
    ...
    3. Alternative - if you get permission errors:
  ...
  EOT
  description = "Complete SSH setup and connection instructions"
  sensitive   = true
}
...
output "ec2_connection_info" {
  value = {
    ...
    ssh_command = "ssh -i ~/.ssh/pni-test-key.pem ec2-user@${aws_instance.test.public_ip}"
    ssh_user    = "ec2-user"
    key_name    = aws_key_pair.main.key_name
  }
  description = "EC2 connection details"
}
