output "resource-ids" {
  ...
  ${data.confluent_service_account_v2.env-manager.display_name}:                     ${data.confluent_service_account_v2.env-manager.id}
  ${data.confluent_service_account_v2.env-manager.display_name}'s Kafka API Key:     "${confluent_api_key_v2.env-manager-kafka-api-key.id}"
  ${data.confluent_service_account_v2.env-manager.display_name}'s Kafka API Secret:  "${confluent_api_key_v2.env-manager-kafka-api-key.secret}"
  ...
  $ confluent kafka topic produce ${confluent_kafka_topic_v3.orders.topic_name} --environment ${data.confluent_environment_v2.staging.id} --cluster ${confluent_kafka_cluster_v2.basic.id} --api-key "${confluent_api_key_v2.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key_v2.app-producer-kafka-api-key.secret}"
  ...
  $ confluent kafka topic consume ${confluent_kafka_topic_v3.orders.topic_name} --from-beginning --environment ${data.confluent_environment_v2.staging.id} --cluster ${confluent_kafka_cluster_v2.basic.id} --api-key "${confluent_api_key_v2.app-consumer-kafka-api-key.id}" --api-secret "${confluent_api_key_v2.app-consumer-kafka-api-key.secret}"
