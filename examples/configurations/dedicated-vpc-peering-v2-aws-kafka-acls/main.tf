...
resource "confluent_network" "peering" {
  ...
  // You could also avoid using dynamic block:
  // zone_info {
  //   zone_id = var.zones_info[0]["zone_id"]
  //   cidr = var.zones_info[0]["cidr"]
  // }

  // zone_info {
  //   zone_id = var.zones_info[1]["zone_id"]
  //   cidr = var.zones_info[1]["cidr"]
  // }

  // zone_info {
  //   zone_id = var.zones_info[2]["zone_id"]
  //   cidr = var.zones_info[2]["cidr"]
  // }
}
...
resource "confluent_peering" "aws" {
  ...
  aws {
    ...
    vpc             = var.vpc_id
    routes          = [data.aws_vpc.customer_vpc.cidr_block]
...
resource "aws_route" "rs" {
  ...
  # https://stackoverflow.com/a/67871267/15013270
  for_each = {
    for index, zone_info in var.zones_info : zone_info.zone_id => zone_info
  }
