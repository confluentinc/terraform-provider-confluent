output "resource-ids" {
  ...
  Kafka Cluster ID: ${confluentcloud_kafka_cluster.basic.id}
  ...
  $ confluent kafka topic produce ${confluentcloud_kafka_topic.orders.topic_name} --environment ${confluentcloud_environment.staging.id} --cluster ${confluentcloud_kafka_cluster.basic.id} --api-key "${confluentcloud_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluentcloud_api_key.app-producer-kafka-api-key.secret}"
  ...
  $ confluent kafka topic consume ${confluentcloud_kafka_topic.orders.topic_name} --from-beginning --environment ${confluentcloud_environment.staging.id} --cluster ${confluentcloud_kafka_cluster.basic.id} --api-key "${confluentcloud_api_key.app-consumer-kafka-api-key.id}" --api-secret "${confluentcloud_api_key.app-consumer-kafka-api-key.secret}"
