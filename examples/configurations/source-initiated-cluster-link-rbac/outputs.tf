output "resource-ids" {
  ...
  * Many of the mirror topic’s configurations are copied and synced from the source topic. A full list is at the end of this page.
  ...
  # 3. Consume records from a mirror topic '${var.source_topic_name}' (created by and owned by a cluster link '${confluent_cluster_link.source-outbound.link_name}') by using ${confluent_service_account.app-manager-destination-cluster.display_name}'s Kafka API Key
