output "instructions" {
  ...
  5. Test connectivity (port 443):
    curl --request GET \
    --url ${confluent_kafka_cluster.dedicated.rest_endpoint}/kafka/v3/clusters/${confluent_kafka_cluster.dedicated.id}/topics \
...
MAINEOF
  ...
  $ confluent kafka topic produce ${local.topic_name} --environment ${confluent_environment.staging.id} --cluster ${confluent_kafka_cluster.dedicated.id} --api-key "${confluent_api_key.app-producer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-producer-kafka-api-key.secret}" --bootstrap "${confluent_kafka_cluster.dedicated.bootstrap_endpoint}"
  ...
  $ confluent kafka topic consume ${local.topic_name} --from-beginning --environment ${confluent_environment.staging.id} --cluster ${confluent_kafka_cluster.dedicated.id} --api-key "${confluent_api_key.app-consumer-kafka-api-key.id}" --api-secret "${confluent_api_key.app-consumer-kafka-api-key.secret}" --bootstrap "${confluent_kafka_cluster.dedicated.bootstrap_endpoint}"
  ...
  3. Update the /etc/hosts file on your laptop (the NGINX proxy was set up via Terraform already):
     echo "\n${aws_instance.test.public_ip} ${trimprefix(trimsuffix(confluent_kafka_cluster.dedicated.rest_endpoint, ":443"), "https://")}" | sudo tee -a /etc/hosts
