...
resource "google_kms_crypto_key" "gcp_key" {
  name     = "byok-key"
...
resource "confluent_byok_key" "gcp_key" {
  gcp {
    key_id = google_kms_crypto_key.gcp_key.id
  }
}

resource "time_sleep" "wait_30_seconds" {
  depends_on = [confluent_byok_key.gcp_key]

  create_duration = "30s"
}

resource "google_kms_crypto_key_iam_member" "crypto_key_encrypter_decrypter" {
  crypto_key_id = google_kms_crypto_key.gcp_key.id
  role          = "roles/cloudkms.cryptoKeyEncrypterDecrypter"
  member        = "group:${confluent_byok_key.gcp_key.gcp[0].security_group}"
}
...
resource "confluent_kafka_cluster" "main" {
  ...
  byok_key {
    id = confluent_byok_key.gcp_key.id
  }

  depends_on = [
    google_kms_crypto_key_iam_member.crypto_key_encrypter_decrypter,
    google_kms_crypto_key_iam_member.crypto_key_viewer,
