...
live-test:
	...
	@if [ -z "$(TF_LIVE_TEST_GROUPS)" ]; then \
		...
		TF_ACC=1 TF_ACC_PROD=1 $(GOCMD) test ./internal/provider/ -v -run=".*Live$$|.*DriftDetection$$" -tags="live_test,all" -timeout 1440m -parallel 15; \
	else \
		...
		TF_ACC=1 TF_ACC_PROD=1 $(GOCMD) test ./internal/provider/ -v -run=".*Live$$|.*DriftDetection$$" -tags="$$TAGS" -timeout 1440m -parallel 15; \
	fi
