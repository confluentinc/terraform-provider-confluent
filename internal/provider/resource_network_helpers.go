...
func networkResourceCustomizeDiff(_ context.Context, diff *schema.ResourceDiff, _ interface{}) error {
	if err := validateNetworkCreateOnly(diff); err != nil {
		return err
	}
...
// in a TODO on connectionTypesSchema(). So the check lives here, where it can also be scoped to
// creation.
var acceptedNetworkConnectionTypes = []string{connectionTypePeering, connectionTypeTransitGateway, connectionTypePrivateLink}

// validateNetworkCreateOnly restores the two cross-field checks the hand-written networkCreate ran
// before building its request: the connection_types closed-set check (verifyListValues) and the
// zones/cloud/connection_types compatibility rule (validateZones). Generated CRUD does not carry
// them, and neither is expressible in the OpenAPI spec, so they belong in CustomizeDiff -- the one
// hook the generator hands back to the provider for exactly this.
//
// Scoped to creation on purpose. The hand-written checks ran in networkCreate only, so a network
// that already exists has never been re-validated. Running them on every plan would reject a
// resource that was imported, or created through the CLI or API before the rule existed -- a
// plan-time failure on infrastructure that is working, which the published provider never produced.
// diff.Id() is empty only for a resource Terraform has not created yet.
//
// The trade against the published behavior is direction, not strictness: these fire at plan instead
// of at apply, so a bad config is caught earlier and nothing is created before it fails.
func validateNetworkCreateOnly(diff *schema.ResourceDiff) error {
	if diff.Id() != "" {
		return nil
	}
