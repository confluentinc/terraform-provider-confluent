...
func TestAccBusinessMetadataBindingDataSourceLive(t *testing.T) {
	...
	if schemaRegistryId == "" || schemaRegistryApiKey == "" || schemaRegistryApiSecret == "" || schemaRegistryRestEndpoint == "" {
		t.Fatal("SCHEMA_REGISTRY_ID, SCHEMA_REGISTRY_API_KEY, SCHEMA_REGISTRY_API_SECRET, and SCHEMA_REGISTRY_REST_ENDPOINT must be set for Business Metadata Binding data source live tests")
	}

	// Generate unique names for test resources to avoid conflicts
	randomSuffix := rand.Intn(100000)
	businessMetadataName := fmt.Sprintf("tf_live_bm_bind_ds_%d", randomSuffix)
	subjectName := fmt.Sprintf("tf-live-subject-bm-bind-ds-%d", randomSuffix)
	...
	businessMetadataBindingDataSourceLabel := "test_live_business_metadata_binding_data"
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_business_metadata_binding.%s", businessMetadataBindingDataSourceLabel), "entity_type", "sr_schema"),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_business_metadata_binding.%s", businessMetadataBindingDataSourceLabel), "schema_registry_cluster.0.id", schemaRegistryId),
					resource.TestCheckResourceAttrSet(fmt.Sprintf("data.confluent_business_metadata_binding.%s", businessMetadataBindingDataSourceLabel), "id"),
					resource.TestCheckResourceAttrSet(fmt.Sprintf("data.confluent_business_metadata_binding.%s", businessMetadataBindingDataSourceLabel), "entity_name"),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_business_metadata_binding.%s", businessMetadataBindingDataSourceLabel), "attributes.owner", "John Doe"),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_business_metadata_binding.%s", businessMetadataBindingDataSourceLabel), "attributes.department", "Engineering"),
				),
			},
		},
	})
...
func testAccCheckBusinessMetadataBindingDataSourceLiveConfig(endpoint, businessMetadataResourceLabel, schemaResourceLabel, businessMetadataBindingResourceLabel, businessMetadataBindingDataSourceLabel, businessMetadataName, subjectName, schemaRegistryId, schemaRegistryRestEndpoint, apiKey, apiSecret, schemaRegistryApiKey, schemaRegistryApiSecret string) string {
	...
	# Read the business metadata binding using data source
	data "confluent_business_metadata_binding" "%s" {
		business_metadata_name = confluent_business_metadata_binding.%s.business_metadata_name
		entity_name           = confluent_business_metadata_binding.%s.entity_name
		entity_type           = confluent_business_metadata_binding.%s.entity_type
