		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fullAwsPeeringResourceLabel, "aws.0.vpc", awsPeeringVpcId),
					resource.TestCheckResourceAttr(fullAwsPeeringResourceLabel, "aws.0.routes.#", "2"),
					resource.TestCheckResourceAttr(fullAwsPeeringResourceLabel, "aws.0.routes.0", awsPeeringRoutes[0]),
					resource.TestCheckResourceAttr(fullAwsPeeringResourceLabel, "aws.0.routes.1", awsPeeringRoutes[1]),
...
func testAccCheckAwsPeeringDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		req := c.networkingV1Client.PeeringsNetworkingV1Api.GetNetworkingV1Peering(c.networkingV1ApiContext(context.Background()), deletedPeeringId).Environment(awsPeeringEnvironmentId)
