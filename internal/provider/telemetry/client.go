...
func (p *sdkPoster) Post(ctx context.Context, u Usage) error {
	...
	if resp != nil && resp.Body != nil {
		// The generated client reads the body it needs; close it so the
		// connection can be reused rather than leaked.
...
func toContractUsage(u Usage) terraformusagev1.TerraformUsageV1Usage {
	...
	// ChangedAttributes is required by the contract; pass the (possibly empty)
	// non-nil slice the wrapper guarantees so it serializes as [] not null.
	m.SetChangedAttributes(u.ChangedAttributes)
	m.SetError(u.Error)
