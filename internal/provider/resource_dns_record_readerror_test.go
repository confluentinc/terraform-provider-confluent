...
// TestAccDnsRecordReadErrorSurfacesResponseBody pins the behavior the regenerated read path
...
func TestAccDnsRecordReadErrorSurfacesResponseBody(t *testing.T) {
	...
	const readErrorScenario = "DnsRecordReadError"
	const stateCreated = "dnsrec-created"
	const stateWaited = "dnsrec-waited"
	const stateReadFailed = "dnsrec-read-failed"
	const readErrorMarker = "DNS_RECORD_READ_501_MARKER_quota_backend_unavailable"

	// Create succeeds and sets the id (dnsrec-abc123, from the fixture).
	...
	_ = wiremockClient.StubFor(wiremock.Post(wiremock.URLPathEqualTo(dnsRecordUrlPath)).
		...
		WillReturn(string(createDnsRecordResponse), contentTypeJSONHeader, http.StatusCreated))

	// This resource is async: Create polls for provisioning before its tail call to Read. The
	// fixture's phase (CREATED) is already a wait target state, so this single poll ends the
	// wait immediately.
	...
	_ = wiremockClient.StubFor(wiremock.Get(wiremock.URLPathEqualTo(dnsRecordReadUrlPath)).
		...
		WillReturn(string(readCreatedDnsRecordResponse), contentTypeJSONHeader, http.StatusOK))
