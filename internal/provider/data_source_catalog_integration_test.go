		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "aws_glue.0.custom_database", "my-custom-database"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "snowflake.0.custom_namespace", "my-custom-namespace"),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "unity.0.custom_schema", "my-custom-schema"),
				),
			},
		},
	})
...
func TestAccDataSourceCatalogIntegrationBigLakeMetastore(t *testing.T) {
	ctx := context.Background()
	...
	readCatalogIntegrationResponse, _ := os.ReadFile("../testdata/catalog_integration/read_created_biglake_metastore_ci.json")
	_ = wiremockClient.StubFor(wiremock.Get(wiremock.URLPathEqualTo("/tableflow/v1/catalog-integrations/tci-abc123")).
		InScenario(CatalogIntegrationDataSourceScenarioName).
		WhenScenarioStateIs(wiremock.ScenarioStateStarted).
		WillReturn(
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "snowflake.#", "0"),
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "unity.#", "0"),
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "biglake_metastore.#", "1"),
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "biglake_metastore.0.provider_integration_id", "cspi-stgce89r7"),
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "biglake_metastore.0.gcp_project_id", "my-gcp-project"),
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "biglake_metastore.0.catalog_name", "catalog_name"),
					resource.TestCheckResourceAttr(CatalogIntegrationResourceName, "biglake_metastore.0.custom_namespace", "my-custom-namespace"),
				),
			},
		},
	})
