...
const (
	envScenarioDataSourceName        = "confluent_environment_v2 Data Source Lifecycle"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckEnvironmentExists(fmt.Sprintf("data.confluent_environment_v2.%s", environmentDataSourceLabel)),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_environment_v2.%s", environmentDataSourceLabel), paramId, environmentId),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_environment_v2.%s", environmentDataSourceLabel), paramDisplayName, environmentDataSourceDisplayName),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_environment_v2.%s", environmentDataSourceLabel), paramId, environmentId),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_environment_v2.%s", environmentDataSourceLabel), paramDisplayName, environmentDataSourceDisplayName),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_environment_v2.%s", environmentDataSourceLabel), paramResourceName, environmentDataSourceEndpoint),
				),
			},
		},
	})
...
func testAccCheckDataSourceEnvironmentConfigWithIdSet(mockServerUrl, environmentDataSourceLabel, environmentId string) string {
	...
	data "confluent_environment_v2" "%s" {
		id = "%s"
	}
