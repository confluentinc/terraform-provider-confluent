...
const (
	...
	aclScenarioName                = "confluent_kafka_acl_v3 Resource Lifecycle"
...
var fullAclResourceLabel = fmt.Sprintf("confluent_kafka_acl_v3.%s", aclResourceLabel)
...
func testAccCheckAclDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "confluent_kafka_acl_v3" {
			continue
		}
...
func testAccCheckAclConfig(confluentCloudBaseUrl, mockServerUrl string) string {
	...
	resource "confluent_kafka_acl_v3" "%s" {
	  kafka_cluster {
        id = "%s"
      }
	  resource_type = "%s"
