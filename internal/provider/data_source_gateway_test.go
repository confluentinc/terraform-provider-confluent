...
func TestAccDataSourceGatewayGcpEgressPrivateLinkGatewaySpec(t *testing.T) {
	ctx := context.Background()
	...
	readGcpEgressPrivateLinkGatewayResponse, _ := os.ReadFile("../testdata/gateway/read_gcp_egress_private_link_gateway.json")
	_ = wiremockClient.StubFor(wiremock.Get(wiremock.URLPathEqualTo("/networking/v1/gateways/gw-gcp123")).
		InScenario(gcpEgressPrivateServiceConnectGatewayScenarioName).
		WhenScenarioStateIs(wiremock.ScenarioStateStarted).
		WillReturn(
			string(readGcpEgressPrivateLinkGatewayResponse),
			contentTypeJSONHeader,
			http.StatusOK,
		))
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fullGatewayResourceName, "aws_peering_gateway.#", "0"),
					resource.TestCheckResourceAttr(fullGatewayResourceName, "aws_egress_private_link_gateway.#", "0"),
					resource.TestCheckResourceAttr(fullGatewayResourceName, "gcp_egress_private_service_connect_gateway.0.region", "us-east-2"),
					resource.TestCheckResourceAttr(fullGatewayResourceName, "gcp_egress_private_service_connect_gateway.0.project", "arn:aws:iam::123456789012:role"),
				),
			},
		},
	})
