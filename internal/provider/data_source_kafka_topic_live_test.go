...
func TestAccKafkaTopicDataSourceLive(t *testing.T) {
	...
	if kafkaClusterId == "" || kafkaApiKey == "" || kafkaApiSecret == "" || kafkaRestEndpoint == "" {
		t.Fatal("KAFKA_STANDARD_AWS_CLUSTER_ID, KAFKA_STANDARD_AWS_API_KEY, KAFKA_STANDARD_AWS_API_SECRET, and KAFKA_STANDARD_AWS_REST_ENDPOINT must be set for Kafka Topic data source live tests")
	}

	// Generate unique names for test resources to avoid conflicts
	randomSuffix := rand.Intn(100000)
	topicName := fmt.Sprintf("tf-live-topic-ds-%d", randomSuffix)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_kafka_topic.%s", topicDataSourceLabel), "topic_name", topicName),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_kafka_topic.%s", topicDataSourceLabel), "partitions_count", "6"),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_kafka_topic.%s", topicDataSourceLabel), "config.cleanup.policy", "delete"),
					resource.TestCheckResourceAttr(fmt.Sprintf("data.confluent_kafka_topic.%s", topicDataSourceLabel), "config.retention.ms", "604800000"),
					resource.TestCheckResourceAttrSet(fmt.Sprintf("data.confluent_kafka_topic.%s", topicDataSourceLabel), "id"),
				),
			},
		},
	})
...
func testAccCheckKafkaTopicDataSourceLiveConfig(endpoint, topicResourceLabel, topicDataSourceLabel, topicName, kafkaClusterId, kafkaRestEndpoint, apiKey, apiSecret, kafkaApiKey, kafkaApiSecret string) string {
	...
	data "confluent_kafka_topic" "%s" {
		...
		topic_name    = confluent_kafka_topic.%s.topic_name
		rest_endpoint = "%s"
	...
	`, endpoint, apiKey, apiSecret, topicResourceLabel, kafkaClusterId, topicName, kafkaRestEndpoint, kafkaApiKey, kafkaApiSecret, topicDataSourceLabel, kafkaClusterId, topicResourceLabel, kafkaRestEndpoint, kafkaApiKey, kafkaApiSecret)
}

func testAccCheckKafkaTopicDataSourceLiveExists(resourceName string) resource.TestCheckFunc {
...
func testAccCheckKafkaTopicDataSourceLiveDestroy(s *terraform.State) error {
