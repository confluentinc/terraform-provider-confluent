...
func TestAccClusterLinkDestinationInbound(t *testing.T) {
	...
	if err := wiremockClient.StubFor(wiremock.Get(wiremock.URLPathEqualTo(readClusterLinkConfigPath)).
		InScenario(clusterLinkScenarioName).
		WhenScenarioStateIs(scenarioStateClusterLinkHasBeenCreated).
		WillReturn(
...
func testAccCheckClusterLinkDestinationDestroy(s *terraform.State, url string) error {
	client := testAccProvider.Meta().(*Client)
	c := client.kafkaRestClientFactory.CreateKafkaRestClient(url, destinationClusterId, destinationClusterApiKey, destinationClusterApiSecret, false, false, client.oauthToken)
