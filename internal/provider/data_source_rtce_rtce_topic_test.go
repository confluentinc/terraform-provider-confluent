...
const (
	rtce_topicDataSourceScenarioName = "confluent_rtce_rtce_topic Data Source Lifecycle"
)
...
func TestAccDataSourceRtceRtceTopic(t *testing.T) {
	...
	kafkaCluster := "kaf-abc123"
	region := "us-west-2"
	topicName := "orders_topic"
	rtce_topicDataSourceLabel := "test_rtce_topic_data_source_label"
	fullRtceTopicDataSourceLabel := fmt.Sprintf("data.confluent_rtce_rtce_topic.%s", rtce_topicDataSourceLabel)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(fullRtceTopicDataSourceLabel, "resource_name", "crn://confluent.cloud/organization=abc/environment=env-abc123/rtce_topic=orders_topic"),
				),
			},
		},
	})
...
func testAccCheckDataSourceRtceRtceTopicConfig(mockServerUrl, rtce_topicDataSourceLabel string, environmentId string, kafkaClusterId string) string {
	...
	data "confluent_rtce_rtce_topic" "%s" {
		topic_name = "%s"
		environment {
			id = "%s"
		}
