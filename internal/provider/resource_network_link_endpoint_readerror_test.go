...
// TestAccNetworkLinkEndpointReadErrorSurfacesResponseBody pins the behavior the regenerated
...
func TestAccNetworkLinkEndpointReadErrorSurfacesResponseBody(t *testing.T) {
	...
	const readErrorScenario = "NetworkLinkEndpointReadError"
	const stateCreated = "nle-created"
	const stateWaited = "nle-waited"
	const stateReadFailed = "nle-read-failed"
	const readErrorMarker = "NETWORK_LINK_ENDPOINT_READ_501_MARKER_quota_backend_unavailable"

	// Create succeeds and sets the id (nle-6wvqx9, from the fixture).
	...
	_ = wiremockClient.StubFor(wiremock.Post(wiremock.URLPathEqualTo(networkLinkEndpointUrlPath)).
		...
		WillReturn(string(createNLEResponse), contentTypeJSONHeader, http.StatusCreated))

	// This resource is async: Create polls for provisioning before its tail call to Read. The
	// fixture's phase (READY) is already a wait target state, so this single poll ends the wait
	// immediately.
	_ = wiremockClient.StubFor(wiremock.Get(wiremock.URLPathEqualTo(networkLinkEndpointReadUrlPath)).
		...
		WillReturn(string(createNLEResponse), contentTypeJSONHeader, http.StatusOK))
	...
	const stateDeleted = "nle-deleted"
