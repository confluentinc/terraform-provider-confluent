...
// TestAccKafkaClientQuotaReadErrorSurfacesResponseBody pins the behavior the regenerated read
...
func TestAccKafkaClientQuotaReadErrorSurfacesResponseBody(t *testing.T) {
	...
	const readErrorScenario = "KafkaClientQuotaReadError"
	const stateCreated = "kcq-created"
	const stateReadFailed = "kcq-read-failed"
	const readErrorMarker = "KAFKA_CLIENT_QUOTA_READ_501_MARKER_quota_backend_unavailable"

	// Create succeeds and sets the id (cq-e857e, from the fixture).
	createKafkaClientQuotaResponse, _ := ioutil.ReadFile("../testdata/kafka_client_quota/create_kafka_client_quota.json")
	_ = wiremockClient.StubFor(wiremock.Post(wiremock.URLPathEqualTo("/kafka-quotas/v1/client-quotas")).
		InScenario(readErrorScenario).
		WhenScenarioStateIs(wiremock.ScenarioStateStarted).
		WillSetStateTo(stateCreated).
		WillReturn(string(createKafkaClientQuotaResponse), contentTypeJSONHeader, http.StatusCreated))
	...
	_ = wiremockClient.StubFor(wiremock.Delete(wiremock.URLPathEqualTo(kafkaClientQuotaUrlPath)).
		InScenario(readErrorScenario).
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckKafkaClientQuotaConfig(mockServerUrl, kafkaClientQuotaResourceLabel, kafkaClientQuotaDisplayName, kafkaClientQuotaDescription,
					kafkaClientQuotaPrincipals, kafkaClientQuotaIngressByteRate, kafkaClientQuotaEgressByteRate),
