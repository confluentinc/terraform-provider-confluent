		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRtceTopicLiveExists(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel)),
					resource.TestCheckResourceAttr(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "cloud", cloud),
					resource.TestCheckResourceAttr(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "description", description),
					resource.TestCheckResourceAttrPair(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "environment.0.id", "confluent_environment.prerequisite", "id"),
					resource.TestCheckResourceAttrPair(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "kafka_cluster.0.id", "confluent_kafka_cluster.prerequisite", "id"),
					resource.TestCheckResourceAttr(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "region", region),
					resource.TestCheckResourceAttrPair(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "topic_name", "confluent_kafka_topic.prerequisite", "topic_name"),
					resource.TestCheckResourceAttrSet(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "id"),
				),
			},
			{
				ResourceName:      fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel),
				ImportState:       true,
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRtceTopicLiveExists(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel)),
					resource.TestCheckResourceAttr(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "cloud", cloud),
					resource.TestCheckResourceAttr(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "description", descriptionUpdated),
					resource.TestCheckResourceAttrPair(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "environment.0.id", "confluent_environment.prerequisite", "id"),
					resource.TestCheckResourceAttrPair(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "kafka_cluster.0.id", "confluent_kafka_cluster.prerequisite", "id"),
					resource.TestCheckResourceAttr(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "region", region),
					resource.TestCheckResourceAttrPair(fmt.Sprintf("confluent_rtce_topic.%s", rtceTopicResourceLabel), "topic_name", "confluent_kafka_topic.prerequisite", "topic_name"),
