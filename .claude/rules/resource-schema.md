...
A growing share of these files are emitted by `cli-terraform-generator` — recognizable by a
`// Code generated by cli-terraform-generator ...; DO NOT EDIT.` header (the PR also touches
`.tf-generation-checksum`). **Do not hand-edit a generated file**: the change is lost on the next
regenerate. Fixes go upstream — a template bug to `cli-terraform-generator`, a wrong/missing
validator or an odd attribute description to the OpenAPI minispec (the generator derives `ValidateFunc`
and descriptions faithfully from the spec, so `StringLenBetween(0, N)` instead of `StringIsNotEmpty`
usually means the spec omits `minLength`). When a regeneration claims to preserve behavior, verify it
against `master` rather than trusting the claim; `GetOkExists` vs `GetOk` and dropped element
attributes are where real regressions have slipped through.
...
- **`DiffSuppressFunc`** where the server canonicalizes a value the user supplies (case-folding a
  region or enum with `strings.EqualFold`, normalizing a query, etc.). Without it, a benign
  server-side rewrite shows as permanent drift; with it added spuriously, a real user change gets
  hidden. Mirror the sibling that suppresses the same kind of value (e.g. `resource_kafka_cluster.go`
  for region case-insensitivity).
- **Nested blocks** use the `Elem: &schema.Resource{...}` (or `&schema.Schema{...}`) pattern already
  used by siblings; match `MaxItems`/`MinItems` conventions. A `TypeList`/`TypeSet` with a missing
  `Elem`, or a dropped `Optional` on an element, changes the set hash — a common regression in
  regenerated code.
...
Repeated literals belong in `constants.go`, not inlined — page sizes, shared enum value lists, poll
intervals/timeouts, and error/deprecation messages. The convention is near-universal (e.g. every
`PageSize` lives in `constants.go`), so an inlined literal that duplicates an existing constant is a
review flag. Live-test-only constants (hardcoded org/env IDs, CRN patterns) go in a
`constants_live_test.go`.
