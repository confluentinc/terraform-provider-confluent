...
- `normalize` - (Required Boolean) If `true`, then schemas are automatically normalized when registered or when passed during lookups.
- `alias` - (Required String) The subject name that this subject is an alias for. 
