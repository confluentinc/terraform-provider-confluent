...
- `dedicated` - (Optional Configuration Block) The configuration of the Dedicated Kafka cluster. It supports the following:
  ...
  - `zones` - (Optional List of String) The list of zones the cluster is in. Zone selection is only supported for `SINGLE_ZONE` dedicated clusters on private network.
