...
- `deletion_protection` - (Optional Boolean) Whether deletion protection is enabled for the Kafka cluster. When set to `true`, the Kafka cluster cannot be deleted until this attribute is set back to `false`. Defaults to `false`.
...
The following end-to-end examples might help to get started with `confluent_kafka_cluster` resource:
  ...
  * [freight-aws-kafka-rbac](https://github.com/confluentinc/terraform-provider-confluent/tree/master/examples/configurations/freight-aws-kafka-rbac): _Freight_ Kafka cluster on AWS that is accessible via Confluent Private Network Interface (PNI) with authorization using RBAC
