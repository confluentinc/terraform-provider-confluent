...
`active_member` chooses which member is active when the pair is created. After that the Switchover service owns it: a failover (the Switchover API's `:failover` operation, for example `confluent switchover pair failover <id> --active-member <name>`) changes the active member on the server, and the resource reads the current value back without showing drift or forcing a replacement. Editing `active_member` in the configuration of an existing pair has no effect — the difference is suppressed, and no plan is produced. See the [complete example](https://github.com/confluentinc/terraform-provider-confluent/tree/master/examples/configurations/switchover-pair).
...
- `active_member` - (Required String) The name of the member that is active when the pair is created; must match one of the `members[].name` values. After creation the Switchover service owns this value: it is read back from the API and always reflects the currently active member, and changing it in the configuration has no effect (no plan is produced). To change which member is active, trigger a failover through the Switchover API's `:failover` operation (for example with the Confluent CLI).
...
- `first_active` - (Required String) The name of the member that was active when the pair was first created. Compare with `active_member` to tell whether the pair is currently failed over.
...
On import, `active_member` is read from the API and any value in your configuration is accepted without a planned replacement; use the currently active member (or `first_active`) for clarity.
