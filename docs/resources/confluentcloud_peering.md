...
page_title: "confluentcloud_peering Resource - terraform-provider-confluentcloud"
subcategory: ""
description: |-
  
---

# confluentcloud_peering Resource

`confluentcloud_peering` provides a Peering resource that enables creating, editing, and deleting Peerings on Confluent Cloud.
...
resource "confluentcloud_network" "aws-peering" {
...
resource "confluentcloud_peering" "aws" {
  ...
  network {
    id = confluentcloud_network.aws-peering.id
  }
}
```
...
resource "confluentcloud_network" "azure-peering" {
...
resource "confluentcloud_peering" "azure" {
  ...
  network {
    id = confluentcloud_network.azure-peering.id
  }
}
```
...
resource "confluentcloud_network" "gcp-peering" {
...
resource "confluentcloud_peering" "gcp" {
  ...
  network {
    id = confluentcloud_network.gcp-peering.id
  }
}
```
...
- `gcp` - (Optional Configuration Block) The Azure-specific Peering details if available. It supports the following:
  ...
  - `custom_routes` - (Optional Boolean) The Import Custom Routes option enables connectivity to a Confluent Cloud cluster in Google Cloud from customer premise or other clouds, such as AWS and Azure, through a customer VPC that is peered with Confluent Cloud in the same region. Defaults to `false`. Learn more about considerations / limitations of the Import Custom Routes option [here](https://docs.confluent.io/cloud/current/networking/peering/gcp-peering.html#import-custom-routes).
...
$ terraform import confluentcloud_peering.my_peer env-abc123/peer-abc123
```
