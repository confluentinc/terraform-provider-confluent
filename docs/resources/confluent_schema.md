...
-> **Note:** When using Option #1 (managing the latest schema version only), Terraform drift can occur when evolving a schema to a version that existed previously. See this [comment](https://github.com/confluentinc/terraform-provider-confluent/issues/619#issuecomment-2765360562) for a workaround. This also applies to trying to delete a ruleset which results in an already existing schema.
