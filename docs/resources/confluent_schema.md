...
-> **Note:** By default, subjects are created in a `default` context. If you want subjects to be created in a specific context, use the following naming pattern: `:.contextName.:subjectName`. For example, use `subject_name = ":.context1.:test-subject"` to create a subject named `test-subject` in the `context1` context.
