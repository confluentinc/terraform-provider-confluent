...
- `recreate_on_update` - (Optional Boolean) An optional flag to control whether a schema should be recreated on an update. Set it to `true` if you want to manage different schema versions using different resource instances. Must be set to the target value when importing. Defaults to `false` which manages the latest schema version only (resource instance always points to the latest schema version by supporting in-place updates).
