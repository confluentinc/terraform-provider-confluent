...
- `skip_validate_during_plan` - (Optional Boolean) An optional flag to control whether a schema should be validated during `terraform plan`. Set it to `true` if you want to skip schema validation during `terraform plan`. Defaults to `false`. Regardless of `true` or `false` for this flag, schema validation will be performed during `terraform apply`. 
